...
export default function Dashboard() {
  ...
  const settings = trpc.milk.settings.useQuery(undefined, { enabled: !!user })
  const monthData = trpc.milk.month.useQuery({ month }, { enabled: !!user })
  const history = trpc.milk.history.useQuery(undefined, { enabled: !!user })

  const invalidate = () => {
    utils.milk.month.invalidate()
    utils.milk.history.invalidate()
    utils.milk.settings.invalidate()
  }
      ...
      <div className="relative max-w-lg mx-auto px-4 sm:px-6">
        ...
        <section className="liquid-glass rounded-3xl p-5 sm:p-6 flex items-center gap-5">
          ...
          <div className="flex-1 min-w-0">
            ...
            <button
              onClick={() => setPaid.mutate({ month, paid: !paid })}
              disabled={setPaid.isPending || totalMl === 0}
              className={
                paid
        ...
        <section className="liquid-glass rounded-3xl p-5 sm:p-6 mt-4">
          ...
          <div className="grid grid-cols-4 gap-2 mt-4">
            {PRESETS.map((ml) => (
              <button
                key={ml}
                onClick={() =>
                  addEntry.mutate({ entryDate: selectedDate, quantityMl: ml })
                }
          ...
          <div className="flex gap-2 mt-3">
            <div className="flex-1 flex items-center rounded-2xl border border-[#f5eedc]/20 bg-[#f5eedc]/5 px-4">
              <input
                ...
                onKeyDown={(e) => {
                  ...
                  if (e.key === 'Enter' && v > 0)
                    addEntry.mutate({ entryDate: selectedDate, quantityMl: v })
                }}
            ...
            <button
              onClick={() => {
                const v = parseInt(customMl, 10)
                if (v > 0) addEntry.mutate({ entryDate: selectedDate, quantityMl: v })
              }}
              disabled={addEntry.isPending || !(parseInt(customMl, 10) > 0)}
              ...
              {history.data!
                ...
                .map((h) => (
                  <button
                    ...
                    className="w-full flex items-center justify-between rounded-xl bg-[#f5eedc]/[0.05] border border-[#f5eedc]/10 px-4 py-3 text-left active:scale-[0.98] transition-transform"
                  >
